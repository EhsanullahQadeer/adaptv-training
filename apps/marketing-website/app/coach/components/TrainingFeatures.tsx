		...
		<div>
			<div className="sm:flex hidden">
				{selectedTab === '1' && (
					<div>
						{' '}
						<Image src={train1} width={1400} height={1000} alt="Build & launch your coaching services"></Image>
					</div>
				)}
				{selectedTab === '2' && (
					<div>
						{' '}
						<Image src={train2} width={1400} height={1000} alt="Build & launch your coaching services"></Image>
					</div>
				)}
				{selectedTab === '3' && (
					<div>
						<Image src={train3} width={1400} height={1000} alt="Build & launch your coaching services"></Image>
					</div>
				)}
			</div>
				...
				{tabData.map((tab) => (
					<div
						...
						className={`w-full cursor-pointer py-8 px-4 border-t-2 transition-all duration-300 ${
							selectedTab === tab.id ? 'border-[#5271FF] bg-[#F2F4F8]' : 'border-[#E7E7E7]'
						}`}
					...
					>
						<div className="flex sm:flex-row flex-col items-baseline gap-4">
							<div>
								<span
									className={`rounded-full w-8 h-8 flex items-center justify-center font-semibold ${
										selectedTab === tab.id ? 'text-white bg-[#5271FF]' : 'text-[#000000] bg-[#E7E7E7]'
									}`}
								>
									{tab.id}
								</span>
							...
							<div className="flex flex-col gap-2">
								<Typography as="h6" fontWeight="font-semibold" sizeVariant="large" color="text-[#000000]">
									{tab.title}
								</Typography>
								<Typography as="p_secondary" sizeVariant="small" color="text-[#475467]">
									{tab.desc}
								</Typography>
							</div>
						</div>
						...
						{selectedTab === tab.id && (
							<div className="sm:hidden mt-4">
								<Image src={tab.image} width={1400} height={1000} alt="Training feature" />
							</div>
						)}
					</div>
				))}
