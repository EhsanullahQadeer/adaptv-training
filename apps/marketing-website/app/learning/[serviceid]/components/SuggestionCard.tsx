...
const SuggestionCard: React.FC<ServiceCardProps> = ({ category, title, image, dotColor }) => {
 ...
 <span className="px-[6px] w-fit items-center py-[4px] mb-1 bg-[#E8E8E8] flex gap-1 rounded-md">
      ...
      <Typography as={"caption"} sizeVariant='small' color='text-black' fontWeight='font-medium'>{category}</Typography>
      </span>
      <Typography as={"caption"} sizeVariant='large' color='text-black' fontWeight='font-semibold' className=" md:w-36  overflow-hidden line-clamp-1">
          {title}
        </Typography>      <span className="flex items-center gap-1 ">
        <Timer/>
