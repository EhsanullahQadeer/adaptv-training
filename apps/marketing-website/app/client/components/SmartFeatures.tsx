				...
				{selectedTab === '1' && (
					<div>
						...
						<Image
							...
							width={1400}
							height={6}
							alt="Build & launch your coaching services"
						></Image>
					</div>
				)}
				{selectedTab === '2' && (
				...
				{tabData.map((tab) => (
					<div
						...
						className={`w-full cursor-pointer py-5 px-4 border-l-2 transition-all duration-300 ${
							selectedTab === tab.id ? 'border-orange-red bg-[#FFFFFF29]' : 'border-[#FFFFFF52]'
						}`}
					...
					>
						<div className="flex sm:flex-row flex-col items-baseline gap-4">
							<div>
								<span
									className={`rounded-full w-8 h-8 text-white flex items-center justify-center font-semibold ${
										selectedTab === tab.id ? ' bg-orange-red' : ' bg-[#FFFFFF33]'
									}`}
								>
									{tab.id}
								</span>
							...
							<div className="flex flex-col gap-2">
								<Typography as="h6" fontWeight="font-semibold" sizeVariant="large" color="text-white">
									{tab.title}
								</Typography>
								<Typography as="p_secondary" sizeVariant="small" color="text-[#FFFFFFB2]">
									{tab.desc}
								</Typography>
							</div>
						</div>
						...
						{selectedTab === tab.id && (
							<div className="md:hidden flex items-center justify-center  mt-4">
								<Image src={tab.image} className="w-[530px] " width={1400} height={1000} alt="Training feature" />
							</div>
						)}
					</div>
				))}
